 ...
 * 사용자 저장
 */
export async function saveUser(sessionId: string, userData: User): Promise<boolean> {
 ...
 * 사용자 업데이트
 */
export async function updateUser(sessionId: string, updates: UserUpdateData): Promise<boolean> {
    if (!database) return false;
 ...
 * 객실 게스트 정보도 함께 동기화
 */
export async function adminUpdateUser(sessionId: string, updates: UserUpdateData): Promise<boolean> {
    if (!database) return false;
    ...
    if (userData.selectedRoom) {
        ...
        const roomData = roomSnapshot.val() as { guests?: unknown[] | Record<string, unknown> } | null;
        ...
        if (roomData && roomData.guests) {
            ...
            const updatedGuests = guests.map((guest: unknown) => {
                const g = guest as { sessionId: string; [key: string]: unknown };
 ...
 */
export async function deleteUserCompletely(sessionId: string, email: string): Promise<{ success: boolean; message: string }> {
    ...
    try {
        ...
        if (userData && userData.selectedRoom) {
            ...
            if (roomData && roomData.guests) {
                ...
                const filteredGuests = guests.filter((g: unknown) => {
                    const guest = g as { sessionId: string };
        ...
        if (email) {
            ...
            const allowedData = allowedSnapshot.val() as { registered?: boolean } | null;
        ...
        const invitations = invSnapshot.val() as Record<string, { inviterSessionId?: string; acceptorSessionId?: string }> | null || {};
        ...
        const requests = reqSnapshot.val() as Record<string, { fromUserId?: string; toUserId?: string }> | null || {};
 ...
 * 해시 검증
 */
async function verifyHash(input: string, hash: string): Promise<boolean> {
 ...
 * OTP 생성 및 DB 저장
 */
export async function createOtpRequest(email: string): Promise<string> {
    if (!database) {
 ...
 * OTP 검증 (서버 측)
 */
export async function verifyOtpRequest(email: string, inputCode: string): Promise<{ valid: boolean; message?: string }> {
    if (!database) {
    ...
    const data = snapshot.val() as { expiresAt: number; attempts: number; hashedCode: string };
 ...
 */
export function generateSecurePassKey(): string {
