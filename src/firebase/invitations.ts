...
import type { RoommateInvitation, CreateInvitationData, Gender, InvitationStatus } from '../types';
import type { Guest } from '../types';

/**
 * 룸메이트 초대 생성
 */
export async function createRoommateInvitation(
    inviterData: CreateInvitationData,
    inviteeName: string
): Promise<RoommateInvitation | null> {
    if (!database) return null;

    // 초대 대상이 이미 방에 배정되어 있는지 확인
    const allRoomsRef = ref(database, 'rooms');
    const allRoomsSnapshot = await get(allRoomsRef);
    const allRooms = allRoomsSnapshot.val() as Record<string, { guests?: Guest[] | Record<string, Guest> }> | null || {};
    ...
    for (const [roomNumber, roomInfo] of Object.entries(allRooms)) {
        ...
        if (guests.some((g: Guest) => g.name === inviteeName.trim())) {
    ...
    const invitation: RoommateInvitation = {
 ...
 */
export async function cleanupUserInvitations(sessionId: string): Promise<void> {
    if (!database) return;
 ...
 * 대기 중인 초대 확인
 */
export async function checkPendingInvitations(userName: string): Promise<RoommateInvitation[]> {
    if (!database) return [];
    ...
    const pendingInvitations: RoommateInvitation[] = [];
    const now = Date.now();
    ...
    for (const [id, invitation] of Object.entries(data)) {
        ...
        if ((invitation.status === 'accepted' || invitation.status === 'rejected') && invitation.createdAt) {
            ...
            if (completedAt && (now - completedAt) > INVITATION_EXPIRY_MS) {
                set(ref(database, `roommateInvitations/${id}`), null).catch((error) => {
...
export async function acceptInvitation(
    invitationId: string,
    acceptorData: { sessionId: string; gender: Gender; name: string },
    ...
    const invitation = snapshot.val() as RoommateInvitation | null;
    ...
    for (const [existingRoom, roomInfo] of Object.entries(allRooms)) {
        ...
        if (guests.some((g: Guest) => g.sessionId === acceptorData.sessionId)) {
 ...
 */
export async function markInvitationNotified(invitationId: string): Promise<boolean> {
    if (!database) return false;
 ...
 * 내가 보낸 초대 취소
 */
export async function cancelInvitation(invitationId: string, inviterSessionId: string): Promise<boolean> {
    if (!database) return false;
