...
export default function RegistrationModal({ onClose }: RegistrationModalProps) {
    ...
    const [otp, setOtp] = useState<string[]>(['', '', '', '', '', '']); // 6자리
    ...
    const handleSendOTP = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        try {
            ...
            if (!result.valid) {
                setError(result.message || '사용자 인증 실패');
            ...
            if (result.alreadyRegistered && result.user) {
                ...
                if (savedSession) {
                    try {
                        const parsed = JSON.parse(savedSession) as Partial<User> & { email?: string; passKey?: string; passKeyExpires?: number };
                        // 같은 이메일이고 PassKey가 만료되지 않았으면 자동 로그인
                        if (parsed.email === email && parsed.passKey && parsed.passKeyExpires && parsed.passKeyExpires > Date.now()) {
    ...
    const handleVerifyOTP = async (): Promise<void> => {
        ...
        try {
            ...
            if (!otpResult.valid) {
                setError(otpResult.message || 'OTP 인증 실패');
                setIsSubmitting(false);
                return;
            }
            ...
            // 5. 로컬 스토리지 저장 (세션 복구용)
            const sessionUser: Partial<User> & { email: string; passKey: string; passKeyExpires: number } = {
                sessionId: sessionId,
                name: userData.name || '',
                email: email,
