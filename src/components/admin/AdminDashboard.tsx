...
import type { RoomGuestsMap, Guest, RoomChangeRequest, Inquiry, RoomInfo, Gender } from '../../types';
...
type TabType = 'dashboard' | 'rooms' | 'requests' | 'inquiries' | 'history' | 'users' | 'activeUsers';
...
export default function AdminDashboard({
    ...
    // 문의 목록 구독
    useEffect(() => {
        const unsubscribe = subscribeToInquiries((inquiryList: Inquiry[]) => {
    ...
    const handleResolveRequest = async (request: RoomChangeRequest): Promise<void> => {
        try {
            if (request.type === 'cancel' && request.currentRoom && request.userId) {
                ...
                await updateUser(request.userId, { selectedRoom: null, locked: false });
            }
            else if (request.type === 'change' && request.currentRoom && (request as RoomChangeRequest & { targetRoom?: string }).targetRoom && request.userId) {
                            ...
                            {inquiries.slice(0, 3).map(inq => (
                                <div key={inq.id} className="p-3 border rounded-lg flex justify-between items-center cursor-pointer hover:bg-gray-50" onClick={() => setActiveTab('inquiries')}>
                                    <div className="truncate flex-1">
                                        <p className="font-medium text-gray-800 truncate">{inq.subject}</p>
    ...
    return (
        <div className="flex bg-gray-50 min-h-screen">
            <Sidebar activeTab={activeTab} onTabChange={(tab: string) => setActiveTab(tab as TabType)} onLogout={onLogout} />
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex flex-wrap gap-4">
                            ...
                            >
                                ...
                                {floors.map(floor => (
                                    <option key={floor} value={floor.toString()}>{floorInfo[floor].label}</option>
                                ))}
                            </select>
                            <select
                                value={filterStatus}
