 ...
 * @param {Object} me - 현재 로그인한 유저
 * @param {Object} roommate - 방에 있는 기존 유저
 * @returns {Array<string>} 경고 메시지 목록 (없으면 빈 배열)
 */
export function checkCompatibility(me, roommate) {
    const warnings = [];
    ...
    // 2. 코골이 검증
    // snoring values: 'no' (없음), 'yes' (있음)

    // 상대방이 코를 심하게 고는 경우 - 나는 '없음'일 때 경고
    if (roommate.snoring === 'yes' && me.snoring === 'no') {
        warnings.push('룸메이트가 "코골이 심함" 상태입니다.');
    }

    // 내가 코를 심하게 고는 경우 - 상대가 '없음'일 때 경고
