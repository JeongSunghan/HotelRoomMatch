...
/**
 * 주민번호 뒷자리 첫 번째 숫자로 성별 판별
 * @param backDigit - 주민번호 뒷자리 첫 번째 숫자
 * @returns 성별 또는 null (유효하지 않음)
 */
export function getGenderFromResidentId(backDigit: string | number): Gender | null {
 ...
 * @param frontPart - 앞 6자리
 * @returns 유효성 여부
 */
export function validateResidentIdFront(frontPart: string): boolean {
 ...
 * @param backDigit - 뒷자리 첫 번째 숫자
 * @returns 유효성 여부
 */
export function validateResidentIdBack(backDigit: string | number): boolean {
 ...
 * 주민번호 검증 결과
 */
export interface ValidateResidentIdResult {
    valid: boolean;
    gender: Gender | null;
    error?: string;
}
 ...
 * @returns 검증 결과
 */
export function validateResidentId(
    frontPart: string,
    backDigit: string | number
): ValidateResidentIdResult {
...
/**
 * 주민번호에서 출생연도 계산
 * @param frontPart - 앞 6자리 (YYMMDD)
 * @param backDigit - 뒷자리 첫 번째 숫자
 * @returns 출생연도 (4자리) 또는 null
 */
export function getBirthYearFromResidentId(
    frontPart: string,
    backDigit: string | number
): number | null {
 ...
 */
export function getAgeFromResidentId(
    frontPart: string,
    backDigit: string | number
): number | null {
